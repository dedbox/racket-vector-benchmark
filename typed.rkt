...
(define-simple-macro (define-benchmark-cases [name make S V scalar op] ...)
  ...
  (begin
    (provide print-typed-benchmark-index run-typed-benchmark)
    ...
    (define (print-typed-benchmark-index offset)
      (for ([t (in-list (list type ...))]
        ...
        (flush-output)))
    (: run-typed-benchmark (Index Positive-Integer Positive-Integer -> Void))
    (define (run-typed-benchmark offset M N)
      (begin
